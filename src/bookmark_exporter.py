...
"""
        ...
        for folder, folder_bookmarks in sorted(by_folder.items()):
            ...
            for bookmark in sorted(folder_bookmarks, key=lambda b: b.title):
                date_str = f' ADD_DATE="{bookmark.date_added}"' if bookmark.date_added else ''
                html += f'        <DT><A HREF="{bookmark.url}"{date_str}>{bookmark.title}</A>\n'
    ...
    @staticmethod
    def to_markdown(bookmarks: List[Bookmark], output_file: str, group_by_folder: bool = True):
        """
