...
def organize_bookmarks():
    ...
    def categorize(b):
        ...
        elif any(x in u+t for x in ['learn', 'tutorial', 'curso', 'udemy']): return "📚 Cursos"
        elif any(x in t+u for x in ['css', 'tailwind', 'html', 'frontend']): return "🎨 Frontend"
        elif any(x in t+u for x in ['backend', 'api', 'python']): return "⚙️ Backend"
        elif any(x in t+u for x in ['tool', 'convert', 'herramient']): return "🛠️ Tools"
        elif 'tryh4rd' in t+u: return "🚀 TryH4rd"
        elif 'inacap' in t+u: return "🎓 Inacap"
        elif any(x in t+u for x in ['ejercit', 'practice']): return "🏋️ Practice"
        elif 'trabajo' in t+u: return "💼 Trabajo"
        else: return "📁 Misc"
