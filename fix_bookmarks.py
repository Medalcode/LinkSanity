...
def categorize(bookmark):
    ...
    elif any(x in url+title for x in ['learn.microsoft', 'tutorial', 'curso', 'udemy', 'hackerrank', 'leetcode', 'platzi']):
        return "📚 Aprendizaje"
    elif any(x in title+url for x in ['css', 'tailwind', 'frontend', 'html', 'react', 'vue']):
        return "🎨 Frontend"
...
data['roots']['bookmark_bar']['date_modified'] = str(int(datetime.now().timestamp() * 1000000))
