...
Ejemplos de uso:
  ...
  python main.py chrome bookmarks.json --format clean --remove-duplicates --output output.html
  ...
  python main.py chrome bookmarks.json --keyword python --output python_bookmarks.json
    ...
    parser.add_argument(
        'input_file',
        type=str,
        help='Archivo de bookmarks a procesar'
    )
    
    parser.add_argument(
        '-o', '--output',
        type=str,
        help='Archivo de salida (la extensión determina el formato: .json, .html, .md, .csv)'
    )
    ...
    parser.add_argument(
        '--remove-duplicates',
        action='store_true',
        help='Eliminar bookmarks duplicados'
    )
    
    parser.add_argument(
        '--keyword',
        type=str,
        help='Filtrar bookmarks que contengan esta palabra clave'
    )
    
    parser.add_argument(
        '--folder',
        type=str,
        help='Filtrar bookmarks de una carpeta específica'
    )
    
    parser.add_argument(
        '--domain',
        type=str,
        help='Filtrar bookmarks de un dominio específico'
    )
    ...
    if len(grouped) <= 10:
        for folder, folder_bookmarks in sorted(grouped.items(), key=lambda x: len(x[1]), reverse=True):
